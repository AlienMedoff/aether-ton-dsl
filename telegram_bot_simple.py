"""
Telegram Bot для управления Aether OS (простая версия без Unicode)
"""
...
class AetherOSBot:
    ...
    async def get_task_status(self, user_id: int) -> str:
        ...
        for task in user_tasks:
            icon = "[OK]" if task.status == TaskStatus.COMPLETED else "[WORK]" if task.status == TaskStatus.IN_PROGRESS else "[WAIT]"
    ...
    async def run_workflow(self, user_id: int, workflow_type: str = "default"):
        ...
        return f"Workflow '{workflow_id}' запущен! [START]"
...
Workflow типы:
[SEQ] Sequential - fetch -> process -> validate -> report
[PAR] Parallel - task1 + task2 -> merge

Статусы задач:
[WAIT] CREATED - создана
[WORK] IN_PROGRESS - выполняется
[OK] COMPLETED - завершена
[FAIL] FAILED - ошибка
...
Готов к работе!
    ...
    async def cmd_stop(self, user_id: int, args: list = None):
        ...
        if user_id in self.aether_bot.user_tasks:
            ...
            return "[STOP] Все задачи остановлены"
